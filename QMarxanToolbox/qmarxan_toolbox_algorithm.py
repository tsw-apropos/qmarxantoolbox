...
class QMarxanToolboxInputFolders(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if os.path.exists(dest):
            ...
            f = open(iName, 'w', newline='')
            f.write("Input file for Annealing program.\n")
            f.write('\n')
            f.write('This file generated by QMarxan Toolbox 2.0.4\n')
...
#    Keyed = namedtuple("Keyed", ["key", "obj"])
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        # set values
        tsf = open(tempsegfile,'w', newline='')
        inGeom = QgsGeometry()
        ...
        # notify users
        # 
        tsf = open(tempsortedfile,'r', newline='')
        taf = open(tempadjfile,'w', newline='')
        done = False
        pl = ''
        x = 0
        adjFileLen = 0
        ...
        # notify users
        #
        saf = open(tempsortedadjfile,'r', newline='')
        faf = open(self.outFName,'w', newline='')
...
class QMarxanToolboxFeatureFile(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterEnum(
                name=self.TARGET_TYPE,
                description=self.tr('Select type of initial targets'),
                options = ["Proportion","Target Value","Target Occurrences"],
                defaultValue = 0
            )
        )
        #  target value
        self.addParameter(
            QgsProcessingParameterNumber(
                name=self.TARGET_VALUE,
                description=self.tr('Set the initial target value'),
        ...
        self.addParameter(
            QgsProcessingParameterNumber(
                name=self.SPF_VALUE,
                description=self.tr('Set the initial SPF value'),
                type=QgsProcessingParameterNumber.Double,
                defaultValue = 0.0
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        self.targetType = self.parameterAsString(parameters,self.TARGET_TYPE,context)
        self.targetValue = self.parameterAsDouble(parameters,self.TARGET_VALUE,context)   
        self.spfValue = self.parameterAsDouble(parameters,self.SPF_VALUE,context)     
        self.outDir = self.parameterAsString(parameters,self.OUT_DIR,context)
        # create field list
        fldsRef = []
        flds =[]
...
class QMarxanToolboxExportPlanningUnitFile(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        # Step 2 - write file
        #
        tmpf = open(self.outFName, 'w', newline='')
...
class QMarxanToolboxEstimateBLM(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        xCost, xBoundary = self.getCostBoundaryLength('calcCost',scenName,outDir,inDir,'X')
        ...
        try:
            ...
            yCost, yBoundary = self.getCostBoundaryLength('realCost',scenName,outDir,inDir,'Y')
            # Step 3 - Calculate "slope" to determine BLM
            # 3a. (step 1 cost - step 2 cost [actually zero]) / (step 1 boundary - step 2 boundary)
            feedback.setProgress(55)
            calcSlope = self.calculateBLMSlope(xCost,xBoundary,yCost,yBoundary)
            if calcSlope is None:
                feedback.pushInfo('Cannot estimate BLM: X and Y boundary lengths are the same or too close to compare.')
                return{'Status':'Error'}
            # 3b. Set blm to "slope"
        ...
        if status == 'Error':
            feedback.pushInfo(messageText)
            return{'Status':'Error'}
        zCost, zBoundary = self.getCostBoundaryLength('calcCost',scenName,outDir,inDir,'Z')
        bVal = self.calculateBLMSlope(zCost,zBoundary,yCost,yBoundary)
        cVal = self.calculateBLMSlope(xCost,xBoundary,zCost,zBoundary)
        if bVal is None or cVal is None:
            ...
            f.close()
            feedback.pushInfo('Cannot estimate BLM range: point Z duplicates the boundary length of X or Y.')
            return{'Status':'Error'}
        rangeMin = min(bVal,cVal)
        rangeMax = max(bVal,cVal)
        #
        f = open(fName,'w', newline='')
        f.write('BLM Estimation Results\n')
        f.write('\n')
        ...
        f.write('Z: Cost %.01f, Boundary %.01f\n' % (zCost,zBoundary))
        f.write('\n')
        f.write('Follow-up BLM toward the minimum boundary clustered solution (Y-Z): %.14f\n' % bVal)
        f.write('Follow-up BLM toward the minimum cost unclustered solution (X-Z): %.14f\n' % cVal)
        f.write('Estimated sensitive BLM values span %.14f to %.14f\n' % (rangeMin, rangeMax))
        f.close()
        #
        feedback.setProgress(100)
        feedback.pushInfo('\nBLM Estimation completed.\n\nSee blm_estimation.txt in report directory for details.\n')
        return{'Status':'Success'}
    #
    #
    # set BLM value
    ...
    # calculate BLM slope
    #
    def calculateBLMSlope(self,firstCost,firstBoundary,secondCost,secondBoundary):

        boundaryDiff = firstBoundary - secondBoundary
        if math.isclose(boundaryDiff,0.0,rel_tol=1e-9,abs_tol=1e-9):
            return None
        return abs((firstCost - secondCost)/boundaryDiff)
    #
    #
    # get cost and boundary length 
    #
    def getCostBoundaryLength(self,param,scenName,outDir,inDir,pointLabel=None):
        ...
        if bestRow is None:
            raise QgsProcessingException('Marxan summary output does not contain any solution rows')
        if param == 'calcCost':
            cost = bestRow['Cost']
